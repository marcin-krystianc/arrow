...
class FLBADecoder : virtual public TypedDecoder<FLBAType> {
 public:
  ...
  /// per-value pointers and no gaps. Useful when the caller only needs the
  /// raw bytes (e.g. float16, fixed-size decimals) and wants to avoid
  /// materializing FixedLenByteArray pointers.
  ///
